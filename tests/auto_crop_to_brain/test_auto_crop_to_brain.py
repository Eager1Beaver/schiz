...
from src.utils.preprocess_validation import plot_slices, calculate_snr_with_mask, calculate_mse, calculate_psnr, calculate_ssim
...
def main(file_path: str, preprocess: bool) -> tuple[np.ndarray, np.ndarray]:
    ...
    if preprocess:
        resampled_data = resample_image(nii, order=1, mode='reflect', cval=0.0)
...
if __name__ == '__main__':
    ...
    # Calculate metrics
    snr_value = calculate_snr_with_mask(cropped_slices)
    ...
    path_to_save_metrics = f"auto_crop_metrics_{mode}.csv"   
