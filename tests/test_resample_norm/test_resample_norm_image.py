...
def evaluate_raw_images(paths_to_images):
    ...
    for path in paths_to_images:
        ...
        cnr_value = calculate_cnr(normalized_data)

        normalized_data_matched = match_dimensions(original_data, normalized_data)
        mse_value = calculate_mse(original_data, normalized_data_matched)
        psnr_value = calculate_psnr(original_data, normalized_data_matched, mse=mse_value)
        ssim_value = calculate_ssim(original_data, normalized_data_matched)

        relative_psnr_value = calculate_relative_psnr(normalized_data)
        relative_rmse_value = calculate_relative_rmse(normalized_data)
...
def plot_save_selected_slices(path_to_best_image, isSave_slices=False):
    ...
    #ax.set_title(title)
    if isSave_slices:
        save_fig_to = 'best_image_slices_resampled_norm.png'
...
if __name__ == "__main__":
    ...
    path_to_all_results = 'test_resampled_norm_image_all_results.csv'
    path_to_best_image = 'test_resampled_norm_image_best_image.csv'
