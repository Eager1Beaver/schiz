...
from src.utils.preprocess import extract_brain, crop_to_largest_bounding_box, apply_gaussian_smoothing
...
def evaluate_raw_images(paths_to_images):
    ...
    for path in paths_to_images:
        ...
        smoothed_data_matched = match_dimensions(normalized_data, smoothed_normalized_data)

        mse_value = calculate_mse(normalized_data, smoothed_data_matched)
        psnr_value = calculate_psnr(normalized_data, smoothed_data_matched, mse=mse_value)
        ssim_value = calculate_ssim(normalized_data, smoothed_data_matched)

        relative_psnr_value = calculate_relative_psnr(smoothed_data_matched)
        relative_rmse_value = calculate_relative_rmse(smoothed_data_matched)
...
def plot_save_selected_slices(path_to_best_image, isSave_slices=False):
    ...
    #ax.set_title(title)
    if isSave_slices:
        save_fig_to = 'best_image_slices_full_preproc_vs_resample_norm.png'
...
if __name__ == "__main__":
    ...
    path_to_all_results = 'test_full_preproc_image_vs_resample_norm_all_results.csv'
    path_to_best_image = 'test_full_preproc_image_vs_resample_norm_best_image.csv'
