...
def grid_search_smoothing_mse_psnr(file_path):
    """
    ...
    for sigma, truncate, mode, cval, order in param_grid:
        try:
            ...
            mse = calculate_mse(original_data, smoothed_data_matched)
            psnr = calculate_psnr(original_data, smoothed_data_matched, mse)
...
if __name__ == "__main__":
    ...
    best_params, all_results = grid_search_smoothing_mse_psnr(file_path)

    # Save results
    output_file_name = "gaussian_mse_psnr_results.csv"
    best_output_file_name = "gaussian_mse_psnr_best.csv"
